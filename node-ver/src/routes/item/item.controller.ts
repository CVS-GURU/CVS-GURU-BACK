...
exports.getItems = (req: express.Request, res: express.Response) => {
  ...
  try {
    ...
    if (!isEmpty(sort)) {
      // 정렬기준 있을 때,
      if (sort === 'price_desc') sql += ` order by item_price desc`
      // 가격 내림차순
    ...
    pool
      ...
      .then((result: any) => {
        if (result[1].length === 0) {
          ...
          return res.json(
            makeResponseFormat('0000', { CONTENTS: result[0], HITS: result[1].length })
          )
        }
      })
      .catch((err: any) => {
    ...
    pool
      ...
      .then((result: any) => {
        if (result[0].length === 0) {
          ...
          return res.json(
            makeResponseFormat('0000', { CONTENTS: result[0], HITS: result[0].length })
          )
        }
      })
      .catch((err: any) => {
      ...
      pool
        ...
        .then((result: any) => {
          ...
          `
            ...
            pool.transaction(queries).then((result: any) => {
              if (result[0].length === 0) {
                // insert 완료 후 result[0]에 데이터 값 없음
...
exports.getItemComment = (req: express.Request, res: express.Response) => {
  try {
    const { item_id } = req.query
    const sql = `
      select user_id as USER_ID,
            item_score as ITEM_SCORE,
            item_comment as ITEM_COMMENT,
            insert_date as INSERT_DATE,
            update_date as UPDATE_DATE
      from item_rate_user
      where 3=3 and item_id = '${item_id}'
      order by insert_date desc
    `
    const queries: string[] = []
    queries.push(sql)
